...
router.get('/', async (req, res) => {
  try {
    ...
    const news = await prisma.news.findMany({
      ...
      take: limit ? parseInt(limit) : 200, // Show up to 200 articles by default
    });
...
router.get('/stock/:symbol', async (req, res) => {
  try {
    const news = await prisma.news.findMany({
      ...
      orderBy: { publishedAt: 'desc' },
      take: 100, // Show up to 100 articles for specific stock
    });
  ...
  try {
    ...
    if (error.code === 'P2002') {
      return res.status(400).json({ 
        error: 'A news article with this URL already exists. Please use a different URL.' 
      });
    }
